...
func (a *K8sAdapter) Mint(ctx context.Context, source v1alpha2.CredentialSource, credNamespace string, principal auth.Principal, scopes []string) (*auth.Token, error) {
...
func (a *K8sAdapter) Validate(source v1alpha2.CredentialSource) error {
	if source.Type != v1alpha2.CredentialSourceTypeServiceAccount && source.Type != v1alpha2.CredentialSourceTypeKubeconfig {
		return fmt.Errorf("%w: expected source type ServiceAccount or Kubeconfig, got %s", ErrInvalidSource, source.Type)
	}
	if source.SecretRef == nil {
