...
func (b *DefaultBroker) Acquire(ctx context.Context, principal auth.Principal, platform string, scopes []string) (*auth.Token, error) {
	...
	token, err := adapter.Mint(ctx, cred.Spec.Source, cred.Namespace, principal, scopes)
	if err != nil {
...
// Patterns are of the form "agent:<name>", "skill:<name>", or wildcards like "agent:*".
// User principals are not supported — user identity is handled out-of-band via header
// propagation, not via PlatformCredential access policy.
