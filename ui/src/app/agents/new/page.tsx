...
function AgentPageContent({ isEditMode, isViewMode, agentName, agentNamespace }: AgentPageContentProps) {
  ...
  interface FormState {
    ...
    serviceAccountName: string;
    credentialMounts: { credentialName: string; mountPath: string }[];
            ...
            try {
              ...
              if (useDeclarativeForm) {
                ...
                setState((prev) => ({
                  ...
                  credentialMounts: decl?.deployment?.credentialMounts || [],
                  byoImage: "",
                  byoCmd: "",
                  byoArgs: "",
                }));
                ...
                setState((prev) => ({
                  ...
                  credentialMounts: (agent.spec?.byo?.deployment?.credentialMounts as { credentialName: string; mountPath: string }[]) || [],
                }));
              }
            } catch (extractError) {
  ...
  const validateForm = () => {
    ...
    const formData = {
      ...
      credentialMounts: state.credentialMounts.filter(m => m.credentialName && m.mountPath),
    };
      ...
      <div className="min-h-screen p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <fieldset disabled={isFormDisabled} className="space-y-6 min-w-0 border-0 p-0 m-0">
            ...
            {(useDeclarativeAgentFields || showByoFields) && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    <Label className="text-sm mb-2 block">Credential Mounts (optional)</Label>
                    <p className="text-xs mb-2 block text-muted-foreground">
                      Mount PlatformCredential resources as files in the agent container (e.g., kubeconfigs for remote clusters).
                    </p>
                    {state.credentialMounts.map((mount, index) => (
                      <div key={index} className="flex items-center gap-2 mb-2">
                        <Input
                          value={mount.credentialName}
                          onChange={(e) => {
                            const updated = [...state.credentialMounts];
                            updated[index] = { ...updated[index], credentialName: e.target.value };
                            setState(prev => ({ ...prev, credentialMounts: updated }));
                          }}
                          placeholder="PlatformCredential name"
                          className="flex-1"
                          disabled={state.isSubmitting || state.isLoading}
                        />
                        <Input
                          ...
                          onChange={(e) => {
                            const updated = [...state.credentialMounts];
                            updated[index] = { ...updated[index], mountPath: e.target.value };
                            setState(prev => ({ ...prev, credentialMounts: updated }));
                          }}
                          placeholder="/etc/kubeconfig/cluster-name"
                          className="flex-1"
                          disabled={state.isSubmitting || state.isLoading}
                        />
                        <Button
                          ...
                          size="icon"
                          onClick={() => {
                            const updated = state.credentialMounts.filter((_, i) => i !== index);
                            setState(prev => ({ ...prev, credentialMounts: updated }));
                          }}
                          disabled={state.isSubmitting || state.isLoading}
                        >
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => setState(prev => ({ ...prev, credentialMounts: [...prev.credentialMounts, { credentialName: "", mountPath: "" }] }))}
                      disabled={state.isSubmitting || state.isLoading}
                    >
                      <PlusCircle className="h-4 w-4 mr-1" /> Add Credential Mount
                    </Button>
                  </div>
                </CardContent>
              </Card>
