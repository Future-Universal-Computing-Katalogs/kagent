...
argument-hint: "optional: topic to focus on (e.g. 'CRD field', 'E2E debugging', 'fork map', 'CI failure')"
---
...
For comprehensive Kind/cluster deployment via `.local/deploy.sh`, see the **kagent-ci** skill (`/kagent-ci`).
For incremental parallel rebuilds of only changed components, see the **kagent-build** skill (`/kagent-build`).
For post-deployment smoke tests, see the **kagent-test** skill (`/kagent-test`).
