...
name: kagent-feature
description: >
  End-to-end feature development orchestrator for the kagent project. Uses a
  Master-Sub agent pattern to coordinate development, testing, deploy, validation,
  and git cleanup. Use for non-trivial features spanning multiple modules.
  For generic (non-kagent) master/sub work use `goal-driven`; for local→cloud
  spec handoff use `kagent-implement`.
user-invocable: true
...
- Relevant skill context (which sections of kagent-dev, kagent-ci, kagent-test, or kagent-git to follow)
...
**Sub agent prompt template**:
```
You are deploying kagent for end-to-end validation.

## Instructions
Pick the right delegate based on target:

A) Local Kind cluster:
   1. Verify cluster: `kubectl get nodes --context kind-kagent`
      If missing, run `make create-kind-cluster`
   2. Invoke `/kagent-build` to incrementally rebuild changed components
   3. Helm changes  → `make helm-install-provider`
      Code-only     → `kubectl rollout restart deploy/kagent-controller deploy/kagent-ui -n kagent --context kind-kagent`
   4. Wait for pods ready, verify health

B) orc / haas (real cluster):
   Invoke `/kagent-ci <cluster>` — it wraps build + push + deploy + rollout.

Do NOT inline `.local/deploy.sh` here — call the skill so the contract stays
in one place.
...
Invoke the `/kagent-git` skill and follow its workflow. Do not paraphrase or
re-derive the grouping rules here — the skill is the single source of truth.

Acceptance after the skill runs:
- `git diff $ORIGINAL_HEAD HEAD` is empty (no changes lost)
- `git log main..HEAD --oneline` reads cleanly by feature/layer
...
| Phase 2: Test | `kagent-dev` + `kagent-build` | CI failures, E2E debugging, incremental rebuild |
| Phase 3: Deploy | `kagent-ci` | `.local/deploy.sh` to orc/haas, or local Kind helm-install |
| Phase 4: Validate | `kagent-test` + `kagent-dev` | Smoke-test phases, E2E debugging |
