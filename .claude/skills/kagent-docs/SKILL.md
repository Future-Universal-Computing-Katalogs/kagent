...
> **See also:** `kagent-spec` — per-directory `CLAUDE.md` consistency scanner.
> Use `kagent-spec` for the AI-memory layer (one CLAUDE.md per dir-with-subdirs);
> use `kagent-docs` for top-level human-readable docs (architecture, README, CHANGELOG).
